...
AvlNode<KeyType>::RotateTwice(AvlNode<KeyType> * & root, dir_t dir)
{
   ...
   root->mySubtree[otherDir] = oldOtherDirSubtree;
      ...
      // update balances
   root->mySubtree[LEFT]->myBal  = -MAXf(root->myBal, 0);
...
AvlNode<KeyType>::Height() const {
   ...
   return  (1 + MAXf(leftHeight, rightHeight));
}
...
template <class KeyType>
static void
Print(std::ostream & os, const AvlNode<KeyType> * node, int level=0)
{
   if (node == NULL) {
      os << "" << endl;
   } else {
      Print(os, RTREE, node, level);
      if (node->Subtree(RIGHT)  !=  NULL) {
         Print(os, node->Subtree(RIGHT), level+1);
      }
      Print(os, KEY, node, level);
      if (node->Subtree(LEFT)  !=  NULL) {
         Print(os, node->Subtree(LEFT), level+1);
      }
      Print(os, LTREE, node, level);
   }// if non-empty tree
}
...
template <class KeyType>
void
AvlTree<KeyType>::PrintTree(std::ostream & os) const {
   Print(os, myRoot);
}
